 ...
 * <p>
 * <ul>
 * <li> navigation: used from a consumer project to include a navigation dependency
 * towards a producer project. ex:
 * <tt><pre>
 *      consumer.dependencies {
 *          navigation project(':producer')
 *      }
 *  </pre></tt>
 * <li> __&lt;name&gt;Navigation: used internally to create a variant aware configuration
 * on the navigation above. It is used to resolve, via attribute matching, the matching artifact
 * from the consumer project. There is one such configuration per variant.
 * <li> &lt;name&gtNavigation{Api, Implementation, CompileOnly, AnnotationProcessor}: these
 * configurations are used in a producer project to add dependencies
 * to the appropriate configuration. This configurations are used when compiling the navigation
 * source set for a given variant. There is a such a configuration per variant, per build type, per
 * product flavor and a main (name is empty) one. The configurations are taken into account in the
 ...
 * project. There is one such configuration per variant. The variant aware configuration
 * has attibutes that will match __&lt;name&gt;Navigation from the consumer project.
 * </ul>
 */
public class ConfigurationManager {
    ...
    public Configuration maybeCreateClientInternalConfiguration(BaseVariant variant) {
        Configuration internalConfiguration = project.getConfigurations().findByName("__" + variant.getName() + NAVIGATION_CONFIGURATION_RADIX);
        if (internalConfiguration != null) {
            return internalConfiguration;
        }

        internalConfiguration = project.getConfigurations().maybeCreate("__" + variant.getName() + NAVIGATION_CONFIGURATION_RADIX);
    ...
    public Configuration maybeCreateClientPseudoConfiguration() {
        Configuration clientPseudoConfiguration = project.getConfigurations().findByName(NAVIGATION_CONFIGURATION);
